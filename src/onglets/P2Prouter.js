...
const P2Prouter = (props) => {
  ...
  function notifHandler2(event) { 
    ...
    indexDev = bufHex.slice(0,2); // index of End device
    indexDev = parseInt(indexDev, 16);
    statusDev = bufHex.slice(2, 4); // status of End device
    ...
    switch (statusDev) {
      case NONE:
        console.log("Satus = NONE ");
        break;

      case DETECTED:
        console.log("Satus = DETECTED ");
        break;

      case CONNECTING:
        console.log("Satus = CONNECTING ");

        break;

      case DISCOVERING:
        console.log("Satus = DISCOVERING ");
  ...
  function notifHandler(event) {
    ...
    var buf = new Uint8Array(event.target.value.buffer);
    console.log(buf);
    createLogElement(buf, 1, "P2Prouter NOTIFICATION RECEIVED");
    let index = buf[0]; 
  ...
  function displayDevice(index){
    if(devices[index].statusDev_ != NONE){
      ...
      if (myElem === null) {
        ...
        name.innerText = "P2P Server" //+ devices[index].name_; // sets the text inside it to "Make: " + the device's make
  ...
  function remove() {
    var boxToRemove = document.getElementById(0);
...
//<div className="container">
    ...
    <div className='bigBoxDevices'>
      ...
      <div className='headerLine smallBoxDevice' id="headerLine" style={{"display": "none"}}>
        ...
        <p className='pAlert'> Alert Button </p>
